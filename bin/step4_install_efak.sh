...
function install_efak(){
    ...
    [[ -f "$installpath/packages/efak.gz" ]] && scp $installpath/packages/efak.gz $ip:$BASE_PATH/efak/
    [[ -f "$installpath/packages/efak.gz" ]] && ssh $ip "gunzip -c $BASE_PATH/efak/efak.gz | docker load"
